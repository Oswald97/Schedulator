...
class GeneticAlgorithm(QtCore.QThread):
    ...
    def evaluateLunchBreak(self, chromosome):
        ...
        for section in chromosome.data['sections'].values():
            ...
            tempScheduleMap = {key: [13] for key in range(6)}
...
class Chromosome:
    ...
    #             // None = Vacant, False = UnDisponible
    #         ]
    #     },
    #     unplaced: {
    ...
    def buildChromosome(self):
        ...
        for section in sections:
            ...
            for timeslotRow in sections[section][1]:
                sectionTimetable.append([None if day == 'Disponible' else False for day in timeslotRow])
        ...
        for instructor in instructors:
            ...
            for timeslotRow in instructors[instructor][2]:
                instructorTimetable.append([None if day == 'Disponible' else False for day in timeslotRow])
        ...
        for room in rooms:
            ...
            for timeslotRow in rooms[room][2]:
                roomTimetable.append([None if day == 'Disponible' else False for day in timeslotRow])
    ...
    def validateSchedule(self, schedule):
        if not self.isRoomTimeslotDisponible(schedule):
            return 1
        if not self.isInstructorTimeslotDisponible(schedule):
            return 2
        if not self.isSectionTimeslotDisponible(schedule):
            return 3
        return True

    def isRoomTimeslotDisponible(self, schedule):
    ...
    def isSectionTimeslotDisponible(self, schedule):
                ...
                for day in schedule[4]:
                    ...
                    for section in schedule[1]:
                        if section in roomDayTimeslot:
                            return False
        # Check for section UnDisponible times
        for section in schedule[1]:
            for timeslotRow in range(schedule[5], schedule[5] + schedule[6]):
                for day in schedule[4]:
    ...
    def isInstructorTimeslotDisponible(self, schedule):
