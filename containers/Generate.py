...
class Generate:
    ...
    def togglePause(self):
        self.toggleState()
        self.parent.btnPause.setText('Pause' if self.running else 'Continuer')
    ...
    def updateDetails(self, details):
        ...
        self.parent.lblMutation.setText('Taux de Mutation: {}%'.format(details[2]))
        self.parent.lblFitness.setText('Score Moyen: {}%'.format(details[3]))
        self.parent.lblPreviousFitness.setText('Score Moyen Préccédent: {}%'.format(details[4]))
        self.parent.lblHighestFitness.setText('Meilleur Score: {}%'.format(details[5]))
        self.parent.lblLowestFitness.setText('Faible Score: {}%'.format(details[6]))
    ...
    def updateTime(self):
        ...
        self.parent.lblTime.setText('Temps écoulé: {}'.format(self.time.toString('hh:mm:ss')))
    ...
    def stopOperation(self):
        ...
        if len(self.topChromosomes):
            self.parent.btnStop.setText('Voir Résultat')
            self.parent.btnStop.clicked.disconnect(self.stopOperation)
            self.parent.btnStop.clicked.connect(self.dialog.close)
            self.parent.lblCPU.setText('Usage du CPU: Stopped')
            self.parent.lblMemory.setText('Usage de la Mémoire: Stopped')
            self.parent.lblStatus.setText('Status: Arrêt')
    ...
    def updateResource(self, resource):
        ...
        if self.tick == 3:
            ...
            try:
                ...
                pass
        self.parent.lblCPU.setText('Usage du CPU : {}%'.format(resource[0]))
        self.parent.lblMemory.setText('Usage de la Mémoire: {}% - {} MB'.format(resource[1][0], resource[1][1]))
