...
class Section:
    ...
    def setupSubjects(self):
        ...
        model.setHorizontalHeaderLabels(['ID', 'Actif', 'En Commun', 'Code', 'Nom', 'ID commun'])
        tree.setModel(model)
...
class Tree:
    def __init__(self, tree):
        self.tree = tree
        self.model = model = QtGui.QStandardItemModel()
        model.setHorizontalHeaderLabels(['ID', 'Actif', 'Nom', 'Classe fixe', 'Action'])
        tree.setModel(model)
        tree.setColumnHidden(0, True)
    ...
    def display(self):
        ...
        for instr in result:
            ...
            btnEdit = QtWidgets.QPushButton('Modifier', frameEdit)
            btnEdit.clicked.connect(lambda state, id=instr[0]: self.edit(id))
            btnDelete = QtWidgets.QPushButton('Supprimer', frameEdit)
    ...
    def delete(self, id):
        ...
        confirm.setText('Voulez vous vrament supprimer cette entrée?')
        confirm.setWindowTitle('Supprimer')
